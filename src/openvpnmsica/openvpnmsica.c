...
#pragma comment(linker, DLLEXP_EXPORT)
    ...
    msica_arg_seq_init(&seqDeleteWintunDriver);
...
cleanup_hRecord:
...

    {
        ...
        if (uiResult == ERROR_UNKNOWN_COMPONENT)
        {
            iInstalled = INSTALLSTATE_NOTUSED;
            iAction = INSTALLSTATE_NOTUSED;
        }
        else if (uiResult != ERROR_SUCCESS)
        {
            SetLastError(uiResult); /* MSDN does not mention MsiGetComponentState() to set GetLastError(). But we do have an error code. Set last error manually. */
        ...
        {
            /* Wintun is installed, but should be degraded to advertised/removed. Schedule Wintun driver deletition.
             *
             * Note: On adapter removal (product is being uninstalled), we skip on errors.
             * Better a partial uninstallation than no uninstallation at all.
             */

            /* Determine installed wintun.dll path and schedule driver deletion. */
            LPTSTR szDeleteWintunDriver;
            MSIHANDLE hRecordDeleteWintunDriver = MsiCreateRecord(1);
            if (!hRecordDeleteWintunDriver)
            {
                uiResult = ERROR_INVALID_HANDLE;
                msg(M_NONFATAL, "%s: MsiCreateRecord failed", __FUNCTION__);
                break;
            }
            uiResult = MsiRecordSetString(hRecordDeleteWintunDriver, 0, TEXT("\"deleteWintunDriver=[") WINTUN_DIRECTORY TEXT("]wintun.dll\""));
            if (uiResult != ERROR_SUCCESS)
            {
                SetLastError(uiResult); /* MSDN does not mention MsiRecordSetString() to set GetLastError(). But we do have an error code. Set last error manually. */
                msg(M_NONFATAL | M_ERRNO, "%s: MsiRecordSetString failed", __FUNCTION__);
                goto cleanup_hRecordDeleteWintunDriver;
            }
            uiResult = msi_format_record(hInstall, hRecordDeleteWintunDriver, &szDeleteWintunDriver);
            if (uiResult != ERROR_SUCCESS)
            {
                goto cleanup_hRecordDeleteWintunDriver;
            }
            msica_arg_seq_add_tail(&seqDeleteWintunDriver, szDeleteWintunDriver);
            free(szDeleteWintunDriver);
cleanup_hRecordDeleteWintunDriver:
            MsiCloseHandle(hRecordDeleteWintunDriver);
            if (uiResult == ERROR_SUCCESS)
            {
                /* Arrange the amount of tick space to add to the progress indicator. */
                MsiRecordSetInteger(hRecordProg, 1, 3 /* OP3 = Add ticks to the expected total number of progress of the progress bar */);
    ...
    if ((uiResult = setup_sequence(hInstall, TEXT("InstallTUNTAPAdapters"          ), &seqInstall          )) != ERROR_SUCCESS
        ...
        || (uiResult = setup_sequence(hInstall, TEXT("DeleteWintunDriver"               ), &seqDeleteWintunDriver )) != ERROR_SUCCESS)
    {
        goto cleanup_hRecordProg;
    }
...
cleanup_exec_seq:
    ...
    msica_arg_seq_free(&seqDeleteWintunDriver);
    if (bIsCoInitialized)
    {
        CoUninitialize();
    }
...
#pragma comment(linker, DLLEXP_EXPORT)
    ...
    for (int i = 1 /*CommandLineToArgvW injects msiexec.exe as szArg[0]*/; i < nArgs; ++i)
    {
        ...
        else if (wcsncmp(szArg[i], L"deleteWintunDriver=", 19) == 0)
        {
            /* Uninstall Wintun driver. */
            if (init_wintun(szArg[i] + 19, TEXT("")) == ERROR_SUCCESS)
            {
                WintunDeleteDriver();
            }
